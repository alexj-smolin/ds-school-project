...
class Tracker:
    ...
    def run(self):
        ...
        frame_enricher = FrameEnricher(trackParams)
        weights_dir = os.path.join(self.basedir, "models", self.params["model"])
        if self.params["async"]:
            detector = AsyncDetector(
                weights_dir, self.params["conf"], self.params["ratiodev"], self.params["smooth"], trackParams
            )
        else:
        ...
        with mlflow.start_run(run_name=trackParams.obj_name):
